...
def write_script(request, filename):
    ...
    try:
        # ADDED: try/except so a malformed POST body doesn't crash the server
        data = request.body.getvalue()
        # TEMP DEBUG: print the raw POST body so we can see exactly what the
        # board received. Remove this print once the parsing issue is found.
        print("RAW BODY:", repr(data))
        fields = data.split("&")
        form_data = {}
        for field in fields:
            ...
            form_data[key] = value
        # TEMP DEBUG
        print("PARSED KEYS:", list(form_data.keys()))
        ...
        print("Error writing script:", ex)
        try:
            storage.remount("/",readonly=True)
        except Exception:
            pass
        response = response_html.format("Error writing script " + filename + ": " + str(ex))
    return("200 OK",[('Content-Type', 'text/html')], response)
...
def write_new_script(request):
    ...
    if(request.method == 'GET'):
        ...
        try:
            ...
            print("Error writing new script:", ex)
            try:
                storage.remount("/",readonly=True)
            except Exception:
                pass
            response = response_html.format("Error writing script: " + str(ex))
    return("200 OK",[('Content-Type', 'text/html')], response)
...
def run_script(request, filename):
    ...
    #print(response)
    try:
        # ADDED: try/except so a bad line in the payload doesn't crash
        # the whole web server / event loop.
        runScript(filename)
    except Exception as ex:
        print("Error running script:", ex)
        response = response_html.format("Error running script " + filename + ": " + str(ex))
    return("200 OK",[('Content-Type', 'text/html')], response)
...
async def startWebService():
    ...
    wsgiServer = server.WSGIServer(PORT, application=web_app, debug=True)
